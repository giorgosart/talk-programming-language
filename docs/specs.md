...
- Copies the contents of source.txt into a new file called destination.txt
- Overwrites the destination if it already exists
- Errors if the source does not exist (unless in attempt)

```markdown
### 13. Function Parameters and Return Values

Enhances functions with input and output capabilities for more flexible and reusable logic.

#### 1. Defining a Function with Parameters
```plaintext
define greet name
    write "Hello" and name in "greeting.txt"
```

- `name` is a parameter available within the function.
- Multiple parameters are space-separated.

#### 2. Calling a Function with Arguments

```plaintext
call greet with "George"
```

- Values are passed in order, matching the parameter list.

#### 3. Returning a Value from a Function

```plaintext
define fullName first last
    set result to first + " " + last
    return result
```

- `return <value>` exits the function and passes the value back to the caller.

#### 4. Capturing a Return Value

```plaintext
call fullName with "Jane" and "Doe" into name
```

- Stores the returned value in the variable `name`.
